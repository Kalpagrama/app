...
export default {
  ...
  computed: {
    shareEmbedText () {
      return `<iframe width="100%" height="315" src="${location.origin}/link/${this.link.oid}?embed=true" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>`
    }
  },
  methods: {
    shareStart () {
      ...
      if (Platform.is.desktop) {
        this.shareLink = location.origin + '/link/' + this.link.oid
    ...
    shareWithTwitter () {
      ...
      let url = `https://twitter.com/intent/tweet?url=${this.shareLink}&via=Kalpagrama&text=${this.link.name}`
      openURL(url)
    }
  },
  mounted () {
