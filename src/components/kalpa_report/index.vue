  ...
  borderRadius: '20px'
    ...
    .column.full-width.q-py-md.q-px-sm.q-gutter-y-sm.text-white.items-center
      .row.full-width.justify-center.text-white.q-mb-sm.q-pl-lg.q-pr-sm
        ...
        span(:style=`{fontSize: '22px'}`).text-white.text-center.text-bold {{$t('Report')}}
        q-btn(round flat color="white" icon="clear" v-close-popup
            :style=`{
        position: 'relative',
        left: '16%'
          }`)
      q-select(v-model="model" :options="options" outlined color='green' :label= "$t('Report reason')" label-color="white" bg-color='grey-9' options-cover :style=`{width: '300px'}`)
