...
borderRadius: '20px'
  ...
  .column.full-width.q-py-md.q-px-sm.q-gutter-y-sm.text-white.items-center
    .row.full-width.justify-center.text-white.q-mb-sm.q-pl-lg.q-pr-sm
      ...
      span(:style=`{fontSize: '16px'}`).text-white.text-center.text-bold {{$t('Report')}}
      q-btn(round flat color="white" icon="clear" v-close-popup).absolute-top-right
    q-select(v-model="model" dark behavior="dialog" :options="options" outlined color='green' :label="$t('Report reason')" label-color="white" stack-label bg-color='grey-9' options-cover :style=`{width: '300px'}`)
    q-input(v-model="text" outlined :style=`{width: '300px'}` :placeholder="$t('Message..')" color='green' autogrow :input-style="{ color: '#fff' }" bg-color='grey-9')
