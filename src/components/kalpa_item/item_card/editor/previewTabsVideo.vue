      ...
      div(:style=`{height: !$store.state.ui.isMobileTiny ? "240px" : "200px",}`).relative-position.row.full-width.items-start.content-start.justify-center.no-scroll
        ...
        .row.full-width.items-start.content-start.justify-center
          ...
          q-btn(v-if="previewUrl"
            ...
            :label="$t('Удалить')"
            :style=`{}`)
q-file(ref="inputThumb" accept="image/*" @update:model-value="$emit('thumbChanged', $event)" :style=`{display: 'none',}`)
q-file(ref="inputPreview" accept="video/*" @update:model-value="$emit('previewChanged', $event)" :style=`{display: 'none',}`)
</template>
...
export default {
  ...
  watch: {
    'rangeModel.min': {
      async handler (to) {
        if (this.$getRef('video')){
          this.$getRef('video').currentTime = to
          this.$getRef('video').pause()
          this.$emit('rangeModelChanged', this.rangeModel)
        }
      }
    },
    'rangeModel.max': {
      async handler (to) {
        if (this.$getRef('video')){
          this.$getRef('video').currentTime = to
          this.$getRef('video').pause()
          // this.$logT('rangeModel.max', this.rangeModel)
          this.$emit('rangeModelChanged', this.rangeModel)
        }
      }
    },
