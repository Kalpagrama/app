...
.column
  .col.relative-position
    ...
    //- Desktop editor
    div(v-if="mode === 'fullscreen' && $screenProps.isDesktop && pageId !== 'node-editor'").row.full-width.absolute-bottom.justify-center.no-pointer-events
      transition(appear enter-active-class="animated fadeIn " leave-active-class="animated fadeOut")
        div(v-if="player && player.duration > 0" :style=`{ maxWidth: 600+'px', background: 'rgba(35,35,35,0.7)', borderRadius: '20px'}`).row.full-width.all-pointer-events
          page-node-editor(
