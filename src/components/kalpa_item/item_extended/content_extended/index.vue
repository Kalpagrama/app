...
.row.full-width.items-start.content-start.justify-center
  ...
  div(v-else).row.full-width
    // header
    div(v-if="$store.state.ui.isMobile").row.full-width.justify-center.b-30
      div(
    ...
    div(:style=`{position: 'relative'}`).row.full-width
      ...
      content-view(:content="content" :style=`{maxHeight : contentMaxHeight + 'px'}`)
    ...
    .row.full-width
      .col
      q-btn(v-if="content.description" round flat dense icon="expand_more" color="grey-5" @click="pageId='description'" :style=`{zIndex: '100'}`)
    // сферы + суть
    .row.full-width
      .row.full-width.col
        // суть
