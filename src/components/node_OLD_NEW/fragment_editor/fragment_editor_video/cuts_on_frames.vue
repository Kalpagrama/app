...
div(:style=`{position: 'relative', minHeight: '100px'}`).row.full-width.items-center.content-center
  ...
  q-btn(
    ...
    :style=`{position: 'absolute', zIndex: 1000, top: '28px', right: '16px'}`)
  .row.full-width
    div(
  ...
  watch: {
    editing: {
      handler (to, from) {
        ...
        if (to) this.framesWidth = this.$refs.framesScrollWrapper.scrollWidth - this.$refs.framesScrollWrapper.clientWidth
      }
    },
    panning: {
      handler (to, from) {
