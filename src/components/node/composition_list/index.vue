...
div(
  ...
  small(
    ...
    ).text-white.q-pa-xs {{ index + 1 }}/{{ items.length }}
  //- preview
  img(
    v-if="true"
  ...
  //- preview list
  div(
    v-if="true && ctx === 'rubick' && items.length > 1"
    ...
    .row.full-width.no-wrap
      div(
        ...
        div(:style=`{height: '30px'}`).row.full-width
          img(
            :src="c.node.meta.items[0].thumbUrl" draggable="false"
            :style=`{
              borderRadius: '10px', overflow: 'hidden', userSelect: 'none', objectFit: 'contain'}`).fit
        div(:style=`{height: '30px'}`).row.full-width
          img(
            :src="c.node.meta.items[1].thumbUrl" draggable="false"
            :style=`{
              borderRadius: '10px', overflow: 'hidden', userSelect: 'none', objectFit: 'contain'}`).fit
</template>
...
export default {
  ...
  watch: {
    nodeOid: {
      ...
      handler (to, from) {
        ...
        this.compositionsChanged(this.items)
      }
    },
    items: {
      deep: true,
      immediate: true,
      handler (to, from) {
        // this.$log(this.label, 'items CHANGED')
  ...
  methods: {
    ...
    compositionNext (ckey) {
      ...
      // this.$log('compositionNext items.length ', this.items.length)
      let index
      if (ckey === 'next' && this.index + 1 < this.items.length) {
        index = this.index + 1
      }
    ...
    async compositionGet (c) {
      ...
      c.composition = nodeFull.items[c.compositionIndex]
    }
  }
}
</script>
