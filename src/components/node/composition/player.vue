...
div(
  ...
  :style=`{position: 'relative', minHeight: height+'px', overflow: 'hidden'}`).row.full-width.items-start.content-start
  //- debug
  ...
  small(
    ...
    :style=`{position: 'absolute', zIndex: 400, top: '4px', right: 'calc(50% - 20px)', borderRadius: '10px', background: 'rgba(0,0,0,0.3)',
      userSelect: 'none'}`
  ...
  //- compositions
  div(
    v-for="(c, ci) in compositions" :key="c.preview+'-'+ci"
    v-if="height > 0 && styles.length > 0 && ci >= index-1 && ci <= index+1"
    :style=`{
      position: 'absolute', bottom: 0, borderRadius: '10px', overflow: 'hidden',
      left: ci < index ? 0 : 'null', right: ci > index ? 0 : 'null',
      bottom: 0,
