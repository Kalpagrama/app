<template lang="pug">
  div(
    ...
    graph-view(ref="graphView" :height="700" :graphD3="graph" @nodeDblClick="addAdjacent")
</template>
...
export default {
  ...
  methods: {
    async addAdjacent (node) {
      this.$log('addAdjacent', node)
      this.itemsRes = await this.$rxdb.find({
        selector: {
          rxCollectionEnum: RxCollectionEnum.LST_SPHERE_ITEMS,
          objectTypeEnum: { $in: ['JOINT'] },
          oidSphere: node.oid,
      ...
      this.$refs.graphView.addNodeToGraph(cloneDeep(node), false)
  ...
  async mounted () {
    this.$log('mounted')
    await this.addAdjacent(this.node)
