...
q-page(:style=`{paddingTop: '8px',}`).row.full-width.justify-center
  div(
    v-if="feed"
    :style=`{height: $q.screen.height-105+'px', maxWidth: $store.state.ui.pageWidth+'px',}`).row.full-width.q-px-sm
    //- feed editor...
  ...
  watch: {
    '$route.params.id': {
      ...
      async handler (to, from) {
        ...
        if (to) {
          let feed = await this.$rxdb.get(RxCollectionEnum.WS_BOOKMARK, to)
          // let [feed] = await this.$rxdb.find({ selector: { rxCollectionEnum: RxCollectionEnum.WS_BOOKMARK, id: to } })
