...
q-layout(view="hHh Lpr lff")
  q-header().b-30
    .row.full-width.justify-center
      ...
      //- items
      .row.full-width.q-pt-xs.q-gutter-xs.q-pr-xs
        div(
          v-for="(i,ii) in 2" :key="ii"
          :style=`{
            maxWidth: itemOpened === ii ? '100%' : '50px',
          }`
          ).col
          div(
            v-if="joint.items[ii]"
            @click="itemOpened = ii").row.fit.items-end.content-end
            div(
              :style=`{
              ...
              div(
                ...
                q-icon(name="blur_on" size="24px" color="white")
                span(v-if="itemOpened === ii").text-white {{ joint.items[ii].name }}
            div(
              v-if="true"
              :style=`{
                height: '40px',
